define([
    ...
    $('.kt-tabs-wrap').each(function (a) {
        var ktStartTab = $(this).find(
                '> .kt-tabs-title-list .kt-tab-title-active')
            .attr('data-tab');
        var ktTabsList = $(this).find('> .kt-tabs-title-list')
            .attr({
                role: 'tablist',
            });
        $(this).find(
        ...
        $(this).find('> .kt-tabs-title-list a').each(function (
            b) {
            var tabId = $(this).attr('data-tab');
            var tabName = $(this).parent().attr('id');
            $(this).closest('.kt-tabs-wrap').find(
                '.kt-tabs-content-wrap > .kt-inner-tab-' +
                tabId).attr('aria-labelledby',
                tabName);
        });
        $(this).find('.kt-tabs-content-wrap > .kt-inner-tab-' +
            ktStartTab).attr('aria-hidden', 'false');
        $(this).find(
            '> .kt-tabs-title-list li:not(.kt-tab-title-active) a'
            ).each(function () {
            $(this).attr({
                role: 'tab',
                'aria-selected': 'false',
    ...
    $('.kt-tabs-title-list li a').click(function (e) {
        ...
        $(this).closest('.kt-tabs-title-list').find(
                '.kt-tab-title-active')
        ...
        $(this).closest('.kt-tabs-wrap').removeClass(function (
            index, className) {
            return (className.match(
                /\bkt-active-tab-\S+/g) || []).join(
                ' ');
        }).addClass('kt-active-tab-' + tabId);
        $(this).parent('li').addClass('kt-tab-title-active')
        ...
        $(this).closest('.kt-tabs-wrap').find(
            '.kt-tabs-content-wrap > .kt-inner-tab-' + tabId
            ).attr('aria-hidden', 'false');
        $(this).closest('.kt-tabs-wrap').find(
                '.kt-tabs-content-wrap > .kt-tabs-accordion-title:not(.kt-tabs-accordion-title-' +
                tabId + ')').addClass('kt-tab-title-inactive')
            .removeClass('kt-tab-title-active').attr({
                tabindex: '-1',
                'aria-selected': 'false',
            });
        $(this).closest('.kt-tabs-wrap').find(
            ...
            .removeClass('kt-tab-title-inactive').attr({
                tabindex: '0',
                'aria-selected': 'true',
            });
        var resizeEvent = window.document.createEvent(
            'UIEvents');
        resizeEvent.initUIEvent('resize', true, false, window,
            0);
        window.dispatchEvent(resizeEvent);
        var tabEvent = window.document.createEvent('UIEvents');
        tabEvent.initUIEvent('kadence-tabs-open', true, false,
            window, 0);
        window.dispatchEvent(tabEvent);
    });
    $('.kt-create-accordion').find(
        '> .kt-tabs-title-list .kt-title-item').each(function () {
        ...
        $(this).closest('.kt-tabs-wrap').find(
            ...
            tabId + ' ' + activeclass + ' ' + iconclass +
            ' ' + iconsideclass + '">' + $(this).html() +
            '</div>');
        $(this).closest('.kt-tabs-wrap').find(
            '> .kt-tabs-content-wrap > .kt-tabs-accordion-title-' +
            tabId + '  a').removeAttr('role');
    });
    $('.kt-tabs-accordion-title a').click(function (e) {
        e.preventDefault();
        var tabId = $(this).attr('data-tab');
        if ($(this).parent('.kt-tabs-accordion-title').hasClass(
                'kt-tab-title-active')) {
            $(this).closest('.kt-tabs-wrap').removeClass(
                'kt-active-tab-' + tabId);
            $(this).parent('.kt-tabs-accordion-title')
                .removeClass('kt-tab-title-active').addClass(
    ...
    function kt_anchor_tabs() {
        if (window.location.hash != '') {
            if ($(window.location.hash + '.kt-title-item').length) {
                ...
                $('#' + tabid).closest('.kt-tabs-title-list').find(
                        '.kt-tab-title-active')
                    .addClass('kt-tab-title-inactive')
                    .removeClass('kt-tab-title-active');
                $('#' + tabid).closest('.kt-tabs-wrap').removeClass(
                ...
                $('#' + tabid).addClass('kt-tab-title-active');
                $('#' + tabid).closest('.kt-tabs-wrap').find(
                        '.kt-tabs-accordion-title.kt-tabs-accordion-title-' +
                        tabid).addClass('kt-tab-title-active')
                    .removeClass('kt-tab-title-inactive');
            }
        }
