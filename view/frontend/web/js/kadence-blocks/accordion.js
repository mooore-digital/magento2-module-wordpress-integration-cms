(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module
        .exports = factory() :
    ...
    function _arrayWithoutHoles(arr) {
        if (Array.isArray(arr)) {
            for (var i = 0, arr2 = new Array(arr.length); i < arr
                .length; i++) arr2[i] = arr[i];
    ...
    function _iterableToArray(iter) {
        if (Symbol.iterator in Object(iter) || Object.prototype
            .toString.call(iter) === "[object Arguments]")
    ...
    if (!Array.from) {
        Array.from = function () {
            ...
            var toInteger = function toInteger(value) {
                ...
                return (number > 0 ? 1 : -1) * Math.floor(
                    Math.abs(number));
            };
            ...
            var toLength = function toLength(value) {
                ...
                return Math.min(Math.max(len, 0),
                    maxSafeInteger);
            ...
            return function from(arrayLike
                ...
                var C =
                this; // 2. Let items be ToObject(arrayLike).

                var items = Object(
                arrayLike); // 3. ReturnIfAbrupt(items).
                ...
                var len = toLength(items
                .length); // 13. If IsConstructor(C) is true, then
                // 13. a. Let A be the result of calling the [[Construct]] internal method
                // of C with an argument list containing the single item len.
                // 14. a. Else, Let A be ArrayCreate(len).

                var A = isCallable(C) ? Object(new C(len)) :
                    new Array(len); // 16. Let k be 0.

                var k =
                0; // 17. Repeat, while k < len… (also steps a - h)
    ...
    */
    ...
    (function (document, window) {
        var el = document.body || document.documentElement,
            ...
            prefixAnimation = '',
            prefixTransition = '';
        if (s.WebkitAnimation == '') prefixAnimation =
            '-webkit-';
        if (s.MozAnimation == '') prefixAnimation = '-moz-';
        if (s.OAnimation == '') prefixAnimation = '-o-';
        if (s.WebkitTransition == '') prefixTransition =
            '-webkit-';
            ...
            'onCSSAnimationEnd', {
                value: function value(callback) {
                    ...
                    this.addEventListener(
                        ...
                        runOnce);
                    this.addEventListener(
                        'mozAnimationEnd', runOnce);
                    this.addEventListener(
                        'oAnimationEnd', runOnce);
                    this.addEventListener(
                        'oanimationend', runOnce);
                    this.addEventListener(
                        'animationend', runOnce);
                    if (prefixAnimation == '' && !(
                            'animation' in s) ||
            ...
            'onCSSTransitionEnd', {
                value: function value(callback) {
                    ...
                    this.addEventListener(
                        ...
                        runOnce);
                    this.addEventListener(
                        'mozTransitionEnd', runOnce);
                    this.addEventListener(
                        'oTransitionEnd', runOnce);
                    this.addEventListener(
                        'transitionend', runOnce);
                    this.addEventListener(
                        'transitionend', runOnce);
                    if (prefixTransition == '' && !(
                            'transition' in s) ||
    ...
    var KadenceAccordion =
        ...
        function () {
            function KadenceAccordion(el, options) {
                ...
                var container = typeof el === 'string' ? document
                    .querySelector(el) : el; // If el is not defined
                ...
                options); // Setting getting elements

                this.container =
                container; // Selecting children of the current accordion instance
                // Kadence Edit
                var panes = Array.from(this.container.children);
                var children = [];
                Array.from(panes).forEach(function (pane) {
                    Array.from(pane.children).forEach(
                        function (item) {
                            children.push(item);
                        });
                });
                ...
                var headerParent = children.filter(function (
                header) {
                    return !header.classList.contains(_this2
                        .settings.panelClass.substr(1));
                }); // Creating an array of all DOM nodes that are Accordion headers
                this.headers = headerParent.reduce(function (acc,
                    header) {
                    var _ref;

                    // Gets all the elements that have the headerClass
                    var a = Array.from(header.children)
                    ...
                    // with all the others.
                    acc = (_ref = []).concat.apply(_ref,
                        _toConsumableArray(acc).concat([
                            a
                        ]));
                    return acc;
                },
            []); // Creates an array of all panel elements for this instance of the accordion
                this.panels = children.filter(function (panel) {
                    return panel.classList.contains(_this2
                        .settings.panelClass.substr(1));
                });
                this.toggleEl = this.settings.toggleEl !==
                    undefined ? Array.from(this.container
                        .querySelectorAll(this.settings.toggleEl)) :
                    this
                    .headers; // This is for managing state of the accordion. It by default sets
            ...
            _createClass(KadenceAccordion, [{
                ...
                value: function init() {
                    ...
                    this
                ._setupAttributes(); // Setting up the inital view of the accordion

                    this
                ._initalState(); // Setting the height of each panel

                    this
                .calculateAllPanelsHeight(); // Inserting data-attribute onto each `header`

                    this
                ._insertDataAttrs(); // Adding listeners to headers

                    this
                ._addListeners(); // Adds class to accordion for initalisation
                ...
                value: function _initalState() {
                    ...
                    var headersToOpen = this
                        .settings.openHeadersOnLoad;

                    if (headersToOpen.length) {
                        this._openHeadersOnLoad(
                            headersToOpen);
                ...
                value: function _addListeners() {
                    ...
                    var _this =
                    this; // Adding click event to accordion
                ...
                value: function handleClick(
                    ...
                    var targetHeaderClass = this
                        .settings.headerClass
                        .substr(
                        1); // Checking that the thing that was clicked on was the accordions header

                    if (targetHeader.classList
                        .contains(
                        targetHeaderClass) && this
                        .toggling === false) {
                        this.toggling =
                        true; // Updating states

                        this.setState(
                        headerIndex); // Render DOM as per the updates `this.states` object
                ...
                targetHeaderId) {
                    ...
                    var states = this
                .getState(); // If `this.settings.openMultiplePanels` is false we need to ensure only one panel
                ...
                value: function togglePanel(
                    animationAction, headerIndex) {
                    var _this8 = this;

                    if (animationAction !==
                        undefined && headerIndex !==
                        undefined) {
                        if (animationAction ===
                            'closed') {
                            ...
                            var header = this
                                .headers[
                                    headerIndex];
                            var panelToClose = this
                                .panels[
                                headerIndex]; // 2. Closeing panel
                            if (!panelToClose
                                .classList.contains(
                                    this.settings
                                    .hiddenClass)) {
                ...
                value: function getState() {
                    ...
                    var headerIds = arguments
                        .length > 0 && arguments[
                        0] !== undefined ?
                        arguments[0] : [];

                    if (headerIds.length && Array
                        .isArray(headerIds)) {
                        var states = headerIds.map(
                            function (header) {
                                return _this9
                                    .states[
                                        header];
                ...
                value: function _openHeadersOnLoad() {
                    ...
                    var headersToOpen = arguments
                        .length > 0 && arguments[
                        0] !== undefined ?
                        arguments[0] : [];
                    if (headersToOpen.length &&
                        Array.isArray(headersToOpen)
                        ) {
                        var headers = headersToOpen
                            .filter(function (
                                header) {
                                return header !=
                                    undefined;
                ...
                value: function _setupAttributes() {
                    ...
                    this
                ._setupHeaders(); // Adding ID & aria-labelledby


                    this
                ._setupPanels(); // Inserting data-attribute onto each `header`
                ...
                value: function calculatePanelHeight(
                    panel) {
                    var panelInner = panel
                        .querySelector(this.settings
                            .panelInnerClass);
                    var activeHeight = panelInner
                        .getBoundingClientRect();
                    return panel.setAttribute(
                        'data-panel-height', ""
                        .concat(activeHeight[
                            'height'], "px"));
...
for (var i = 0, len = accordionsArray.length; i < len; i++) {
    var multiplePanels = accordionsArray[i].getAttribute(
        'data-allow-multiple-open');
    var openPanels = accordionsArray[i].getAttribute('data-start-open');
    var openPanel = parseInt(openPanels);
    if (openPanels !== 'none') {
        for (var b = 0, lenb = accordionsArray[i].children.length; b <
            lenb; b++) {
            if (accordionsArray[i].children[b].classList.contains(
                    'kt-accordion-pane-' + (1 + openPanel))) {
                openPanel = b;
            }
        }
    }
    new KadenceAccordion(accordionsArray[i], {
        openHeadersOnLoad: (openPanels === 'none' ? [] : [parseInt(
            openPanel)]),
        headerClass: '.kt-blocks-accordion-header',
        panelClass: '.kt-accordion-panel',
        panelInnerClass: '.kt-accordion-panel-inner',
            ...
            if (window.location.hash != '') {
                ...
                if (element) {
                    if (element.classList.contains(
                        ...
                        var child = document.querySelectorAll('#' + id +
                            ' .kt-blocks-accordion-header')[0];
                        if (!child.classList.contains(
                                'kt-accordion-panel-active')) {
                            child.click();
                        }
                        window.setTimeout(function () {
                            kt_accordion_scrollTo(document.body,
