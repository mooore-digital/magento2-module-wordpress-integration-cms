...
class IndexBlogUrls
{
    ...
    public function execute(): void
    {
        ...
        foreach ($sites as $siteId => $site) {
            ...
            foreach ($site['data'] as $post) {
                $db_post = array_filter($dbPosts, function ($row) use ($post, $siteId) {
                ...
                $db_post = $db_post[0];
                if ($db_post->getSlug() !== $post['slug']) {
                    $urls = array_merge($urls, $this->updatePageSlug($db_post, $post['slug']));
                }
            }
        }
        ...
        foreach ($dbPosts as $post) {
            ...
            $is_deleted = array_filter($site['data'], function ($row) use ($post) {
    ...
    public function registerPage(array $page)
    {
        ...
        $model->setSlug($slug);
        $model_id = $this->wordpressPostRepository->save($model);

        return $this->createRewriteRules($siteId, $postId, $model_id, $slug);
    }
    ...
    public function createRewriteRules($siteId, $postId, $entity_id, $slug)
    {
